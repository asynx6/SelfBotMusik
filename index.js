...
async function joinVC(guildId, channelId) {
    ...
    player.connect({ guildId, voiceChannel: channelId, self_deaf: true, self_mute: false });
...
async function leaveVC(force = false) {
    ...
    if (player) {
        try { try { player.queue.length = 0; } catch {} } catch {}
...
function broadcastNow() { stateMod.emitter.emit("changed"); }
...
const ctx = {
    ...
    stopAll: () => playerMod.stopAll(poru, runtimeMod.Runtime),
...
setInterval(() => {
    ...
    if (!p) return;
    stateMod.patch({ position: p.position || 0 });
}, 500);
